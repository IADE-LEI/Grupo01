...
public class Entity {
  ...
  ArrayList<EntityComponent> EntityComponents;
  ...
  public <T extends EntityComponent> T GetFirstComponentOfType(Class<T> ComponentClassType) {
    for (int i = 0; i < EntityComponents.size(); ++i) {
      EntityComponent Component = EntityComponents.get(i);
  ...
  // Add Component
  public <T extends EntityComponent> T AddComponent(EntityComponent ComponentToAdd) {
  ...
  // Remove Component
  public <T extends EntityComponent> void RemoveFirstComponentOfType(Class<T> ComponentClassType) {
    for (int i = 0; i < EntityComponents.size(); ++i) {
      EntityComponent Component = EntityComponents.get(i);
      if (Component != null && ComponentClassType.isAssignableFrom(Component.getClass())) {
  ...
  public Rectangle getBounds() {
    SpriteRendererComponent spriteRendererComponent =
        this.GetFirstComponentOfType(SpriteRendererComponent.class);
    if (spriteRendererComponent == null) return null;

    return new Rectangle(
        spriteRendererComponent.SpriteToRender.getX() - 0.5f,
        spriteRendererComponent.SpriteToRender.getY() - 0.5f,
        1,
        1);
  }
}
