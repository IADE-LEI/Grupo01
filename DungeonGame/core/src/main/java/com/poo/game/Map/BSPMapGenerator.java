    ...
    private void GeneratePathingFromMapLayout() {
        ...
        for (int Y = 0; Y < mapData.getHeight(); ++Y) {
            for (int X = 0; X < mapData.getWidth(); ++X) {
                ...
                boolean HasLeftNeighbour = Boundaries.IsValidIndex(X - 1, 0, DimensionX - 1) && (MapNodeGrid[X - 1][Y] != null);
                boolean HasTopNeighbour = Boundaries.IsValidIndex(Y - 1, 0, DimensionY - 1) && (MapNodeGrid[X][Y - 1] != null);
                boolean HasRightNeighbour = Boundaries.IsValidIndex(X + 1, 0, DimensionX - 1) && (MapNodeGrid[X + 1][Y] != null);
                boolean HasBottomNeighbour = Boundaries.IsValidIndex(Y + 1, 0, DimensionY - 1) && (MapNodeGrid[X][Y + 1] != null);
                ...
                if (HasBottomNeighbour)
                    MapNodeGrid[X][Y].AddTwoWayConnectionTo(MapNodeGrid[X][Y + 1]);
                mapData.MapGraph.AddNode(MapNodeGrid[X][Y]);
            }
        }
    }
}
